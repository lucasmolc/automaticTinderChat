...
- **Backend (organização)**: app factory (`web/factory.py`) e estado compartilhado centralizado (`web/extensions.py`); rotas auxiliares (IA, notificações, logs, A/B, ML, scheduler, cache, auditoria, tarefas, métricas) movidas para `web/blueprints/api_extra.py`. `web/app.py` reduzido de ~3.300 para ~1.885 linhas, sem alterar os testes (405 verdes).
