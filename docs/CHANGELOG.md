...
- **Backend (organização)**: app factory (`web/factory.py`) e estado compartilhado centralizado (`web/extensions.py`); **todas as rotas movidas para blueprints** (`web/blueprints/`: `matches_messages`, `operations`, `api_extra`, `fragments`). `web/app.py` reduzido de ~3.300 para ~215 linhas (apenas factory, páginas, health e `run_web_server`), sem alterar os testes (405 verdes).
- **UI (Fase 2, início)**: HTMX e Alpine.js adicionados; estatísticas do dashboard migradas para fragmento HTMX (`/fragments/stats` + `partials/_stats.html`), com helper `compute_dashboard_stats()` compartilhado com `/api/stats` (DRY). Demais páginas/modais nativos: em andamento (requer validação no navegador).
